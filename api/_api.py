...
def request_profile(handler):
    ...
    rows = _rest_rows(
        "users",
        {"select": "id,auth_user_id,email,role,is_active", "auth_user_id": f"eq.{auth_user_id}", "limit": "1"},
    )
    if not rows and email:
        rows = _rest_rows(
            "users",
            {"select": "id,auth_user_id,email,role,is_active", "email": f"eq.{email}", "limit": "1"},
        )
...
def profile_can_access_task(profile, task_id):
    ...
    rows = _rest_rows(
        "order_tasks",
        {"select": "id", "id": f"eq.{task_id}", "assigned_user_id": f"eq.{profile.get('id')}", "limit": "1"},
    )
    return bool(rows)
...
def profile_can_access_order(profile, order_id):
    ...
    rows = _rest_rows(
        "order_tasks",
        {"select": "id", "order_id": f"eq.{order_id}", "assigned_user_id": f"eq.{profile.get('id')}", "limit": "1"},
    )
    return bool(rows)
